...
class GraphLayer(nn.Module):
    ...
    def forward(self, label_emb, extra_attn):
        ...
        if self.graph_type == 'graphormer':
            ...
            label_emb = self.graph(label_emb.squeeze(0), hyperedge_index=extra_attn)
            label_emb = nn.functional.dropout(label_emb, p=self.dropout, training=self.training)
            label_emb = residual + label_emb
            label_emb = self.layer_norm(label_emb)
        else:
...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph_type='GAT', layer=1, path_list=None, data_path=None):
        ...
        if self.graph_type == 'graphormer':
            ...
            print("Constructing hypergraph structure...")
            # Constructing hyperedges from ath_list (parent-child pairs)
            # The format of path_list is [(parent, child), ...]
            parent_to_children = defaultdict(list)
            for parent, child in path_list:
                # Only handle the actual label hierarchy relationship
                if parent < self.label_num and child < self.label_num:
            ...
            # Each superedge = 1 parent node + N child nodes
            for parent, children in parent_to_children.items():
                if not children:
                    continue
                
                # Add the parent node and all child nodes to the superedge.
                hyperedge_nodes = [parent] + children
            ...
            print(f"Hypergraph constructed with {self.label_num} nodes and {hyperedge_id_counter} hyperedges.")
        else:
